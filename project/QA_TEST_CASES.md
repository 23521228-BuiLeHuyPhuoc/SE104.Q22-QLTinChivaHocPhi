...
| BUG-005 | classController.js | Critical | Business rule | Công thức trùng lịch chưa thống nhất với quy tắc chạm biên | SQL cũ dùng inclusive ở một số nơi | Dùng overlap `[start, effectiveEnd)`, cho phép chạm biên | Có |
...
| BUG-005 | classController.js | Sửa overlap lịch nửa mở | Chặn trùng lịch thật, cho phép chạm biên | Conflict đúng quy tắc tiết | TC Admin / Classes |
...
| TC-018 | Student / Course Registration | Trùng lịch | Business/Negative | Đã có lớp cùng thứ và khoảng tiết giao thật | Đăng ký lớp overlap thật | Đăng ký lớp overlap thật | API từ chối trùng lịch; khoảng chỉ chạm biên như `1-3` và `3-5` được phép | High | Playwright + Supertest; seed DB theo tiền điều kiện |
...
| TC-236 | Admin / Classes | Validate lịch overlap nửa mở | Business rule/Regression | Có phòng/giảng viên/lớp bị overlap thật | Cùng thứ và tiết giao thật; ca riêng chỉ chạm biên | Tạo/sửa lớp hoặc validate-schedule | API từ chối overlap thật và cho phép chạm biên, regression BUG-005 | Critical | Supertest + Playwright; seed/mocks theo tiền điều kiện |
