...
Tài liệu này mô tả cấu trúc database sau khi chạy đầy đủ file `init.sql`, bao gồm phần tạo bảng ban đầu, dữ liệu mẫu, các cột bổ sung trong phần `SCHEMA COMPATIBILITY`, chỉ mục, function và trigger hiện có.
...
| Schema compatibility | Bổ sung thêm cột bằng `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` để tương thích với ứng dụng hiện tại. |
...
| `DaXoa`, `NguoiXoa`, `NgayXoa` | Nhóm cột xóa mềm, được bổ sung cho một số bảng. |
...
| `TrangThai` | `BOOLEAN` | Trạng thái hoạt động, được bổ sung sau khởi tạo. |
| `NgayTao` | `TIMESTAMP` | Thời điểm tạo, được bổ sung sau khởi tạo. |
| `NguoiCapNhat`, `NgayCapNhat` | `INTEGER`, `TIMESTAMP` | Thông tin audit cập nhật, được bổ sung trong nhóm audit. |
| `DaXoa`, `NguoiXoa`, `NgayXoa` | `BOOLEAN`, `INTEGER`, `TIMESTAMP` | Thông tin xóa mềm, được bổ sung trong nhóm audit. |
...
| `TrangThai`, `NgayTao` | `BOOLEAN`, `TIMESTAMP` | Trạng thái và thời điểm tạo, được bổ sung sau khởi tạo. |
...
| `DiaChi` | `VARCHAR(200)` | Địa chỉ/khu nhà của khoa, được bổ sung sau khởi tạo. |
| `TruongKhoa` | `VARCHAR(100)` | Họ tên trưởng khoa, được bổ sung sau khởi tạo. |
...
| `LanDangNhapCuoi` | `TIMESTAMP` | Lần đăng nhập gần nhất, được bổ sung sau khởi tạo. |
| `RefreshToken` | `VARCHAR(500)` | Refresh token đăng nhập, được bổ sung sau khởi tạo. |
...
| `Cccd` | `VARCHAR(20)` | Căn cước công dân, duy nhất; sau compatibility có thể được đặt NOT NULL nếu dữ liệu đủ. |
| `MaPhuongXa` | `VARCHAR(20)` | FK đến `PHUONGXA.MaPhuongXa`. |
| `MaDanToc` | `VARCHAR(10)` | FK đến `DANTOC.MaDanToc`; sau compatibility có thể được đặt NOT NULL nếu dữ liệu đủ. |
| `MaNganh` | `VARCHAR(10)` | FK đến `NGANHHOC.MaNganh`. |
| `DiaChiLienHe` | `VARCHAR(200)` | Địa chỉ liên hệ; sau compatibility có thể được đặt NOT NULL nếu dữ liệu đủ. |
| `Sdt` | `VARCHAR(15)` | Số điện thoại. |
...
| `HoTenCha`, `SdtCha`, `HoTenMe`, `SdtMe` | Nhiều kiểu | Thông tin phụ huynh, được bổ sung sau khởi tạo. |
...
| `GhiChu` | `VARCHAR(300)` | Ghi chú. |
| `ThuTu` | `INTEGER` | Thứ tự học kỳ trong năm học, được bổ sung sau khởi tạo. |
| `NgayTao` | `TIMESTAMP` | Thời điểm tạo, được bổ sung sau khởi tạo. |
| `NguoiCapNhat`, `NgayCapNhat`, `DaXoa`, `NguoiXoa`, `NgayXoa` | Nhiều kiểu | Nhóm cột audit/xóa mềm. |
...
| `SoTinChi` | `INTEGER` | Số tín chỉ tại thời điểm đăng ký, được bổ sung sau khởi tạo. |
| `LoaiMon` | `VARCHAR(5)` | Loại môn tại thời điểm đăng ký, được bổ sung sau khởi tạo. |
...
- `PHIEUTHUHOCPHI.TrangThai` sau compatibility chấp nhận 4 trạng thái: `Chờ xác nhận`, `Thành công`, `Thất bại`, `Đã hủy`.
