...
| RBTV09 | Người 3 | `HOCKY`, `NAMHOC` | Học kỳ phải đúng thứ tự và khoảng thời gian. | `LoaiHocKy = 'Chính' -> ThuTu IN (1,2)`; `LoaiHocKy = 'Hè' -> ThuTu = 3`; `UNIQUE(MaNamHoc, ThuTu)`; bắt buộc cấu hình đủ ngày học kỳ, đăng ký và cứu xét; `NgayBatDau < NgayKetThuc`; `NgayBatDauDangKy < NgayKetThucDangKy`; `NgayBatDauDangKy < NgayKetThucDangKy < NgayBatDauCuuXet <= NgayKetThucCuuXet < NgayBatDau`; `HanDongHocPhi >= NgayBatDau`. | `INSERT/UPDATE HOCKY`; `UPDATE NAMHOC` nếu thay đổi năm học ảnh hưởng học kỳ |
...
| RBTV35 | Người 4 | `PHIEUDANGKY`, `PHIEUTHUHOCPHI`, `HOCKY` | Sinh viên có thể đóng học phí nhiều lần cho một phiếu đăng ký nhưng phải hoàn thành trước hạn đóng học phí của học kỳ để đủ điều kiện dự thi cuối kỳ. | Với mỗi phiếu đăng ký: mọi phiếu thu `TrangThai = 'Thành công'` phải có `NgayXacNhan < HOCKY.HanDongHocPhi + 1 ngày`; tổng `SoTienThu` thành công trước hạn phải lớn hơn hoặc bằng `PHIEUDANGKY.TongTienPhaiDong` thì `DuDieuKienThi = TRUE`. Hệ thống cung cấp `fn_rbtv35_du_dieu_kien_thi_hoc_phi(SoPhieu)` và view `V_DIEUKIENDUTHI_HOCPHI`; nếu bổ sung bảng dự thi thì dùng hàm này để chặn lập danh sách dự thi cuối kỳ. | `INSERT/UPDATE PHIEUTHUHOCPHI`; `UPDATE HOCKY.HanDongHocPhi`; `UPDATE PHIEUDANGKY.MaHocKy` |
