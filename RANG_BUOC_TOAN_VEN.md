...
| Người 4 | Học phí, phiếu thu, báo cáo, thông báo, xóa mềm/trạng thái | `RBTV28` - `RBTV35` | Quản lý học phí, phiếu thu, xác nhận thanh toán, báo cáo nợ học phí, thông báo, thùng rác | Xem học phí, tạo yêu cầu thanh toán, xem lịch sử đóng tiền, nhận thông báo | `fn_tinh_tong_tien_da_thu(SoPhieu)`, `fn_tinh_so_tien_con_lai(SoPhieu)`, `fn_rbtv35_du_dieu_kien_thi_hoc_phi(SoPhieu)` |
...
| RBTV16 | Người 1 | `PHIEUDANGKY`, `SINHVIEN`, `NGUOIDUNG` | Chỉ sinh viên còn học và tài khoản còn hoạt động mới được lập phiếu đăng ký. | Với mỗi `PHIEUDANGKY`: tồn tại `SINHVIEN sv` sao cho `sv.MaSv = pdk.MaSv AND sv.TrangThai = 'Đang học'`; nếu sinh viên có tài khoản thì tài khoản phải thỏa `NGUOIDUNG.TrangThai = TRUE`. Không xét `NGUOIDUNG.TrangThaiDuyet` vì trạng thái duyệt không còn dùng trong nghiệp vụ hiện tại. | `INSERT/UPDATE PHIEUDANGKY`; `UPDATE SINHVIEN.TrangThai/MaTaiKhoan`; `UPDATE NGUOIDUNG.TrangThai` |
...
| RBTV09 | Người 3 | `HOCKY`, `NAMHOC` | Học kỳ phải đúng thứ tự và khoảng thời gian. | `LoaiHocKy = 'Chính' -> ThuTu IN (1,2)`; `LoaiHocKy = 'Hè' -> ThuTu = 3`; `UNIQUE(MaNamHoc, ThuTu)`; bắt buộc cấu hình đủ ngày học kỳ, đăng ký, cứu xét và học phí; `NgayBatDau < NgayKetThuc`; `NgayBatDauDangKy < NgayKetThucDangKy`; `NgayBatDauDangKy < NgayKetThucDangKy < NgayBatDauCuuXet <= NgayKetThucCuuXet < NgayBatDau`; `NgayKetThucCuuXet < NgayBatDauDongHocPhi <= HanDongHocPhi < NgayKetThuc`. | `INSERT/UPDATE HOCKY`; `UPDATE NAMHOC` nếu thay đổi năm học ảnh hưởng học kỳ |
...
| RBTV24 | Người 3 | `PHIEUDANGKY`, `CHITIETDANGKY`, `THAMSO`, `MONDAHOC`, `SINHVIEN`, `HOCKY`, `NAMHOC` | Tổng tín chỉ đăng ký trong một học kỳ không được vượt giới hạn hệ thống; riêng sinh viên đã tới học kỳ cuối của năm 2 mà chưa hoàn thành đủ các môn Anh văn bắt buộc thì bị giới hạn 14 tín chỉ hoặc theo tham số tương ứng. | `TongTinChiDangKy = SUM(ctdk.SoTinChi WHERE ctdk.TrangThai = 'Đã đăng ký')`. Phải có `TongTinChiDangKy <= GioiHan`. Mặc định `GioiHan = THAMSO.SoTinChiDangKyToiDa`. Nếu sinh viên đăng ký ở học kỳ cuối của năm học thứ `THAMSO.NamKiemTraAnhVan` tính từ `SINHVIEN.NgayNhapHoc` trở đi, mặc định là học kỳ cuối năm 2, và chưa tồn tại đủ các dòng `MONDAHOC(MaSv, MaMonHoc IN THAMSO.DanhSachMonAnhVanBatBuoc, KetQua = 'qua_mon')`, thì `GioiHan = THAMSO.GioiHanTinChiChuaDatAnhVan` mặc định 14. Nếu project có cơ chế cho phép vượt tín chỉ thì giới hạn cuối cùng vẫn không được vượt `THAMSO.SoTinChiDangKyToiDaKhiVuot`. | `INSERT/UPDATE/DELETE CHITIETDANGKY`; `INSERT/UPDATE PHIEUDANGKY.MaSv/MaHocKy/TrangThai`; `UPDATE THAMSO`; `INSERT/UPDATE/DELETE MONDAHOC`; `UPDATE SINHVIEN.NgayNhapHoc`; `UPDATE HOCKY.MaNamHoc/ThuTu/LoaiHocKy`; `UPDATE NAMHOC.NamBatDau` |
...
| RBTV35 | Người 4 | `PHIEUDANGKY`, `PHIEUTHUHOCPHI`, `HOCKY` | Sinh viên có thể đóng học phí nhiều lần cho một phiếu đăng ký nhưng phải đóng trong khung học phí và hoàn thành trước hạn đóng học phí của học kỳ để đủ điều kiện dự thi cuối kỳ. | Với mỗi phiếu đăng ký: mọi phiếu thu `TrangThai = 'Thành công'` phải có `HOCKY.NgayBatDauDongHocPhi <= NgayXacNhan < HOCKY.HanDongHocPhi + 1 ngày`; tổng `SoTienThu` thành công trong khung học phí phải lớn hơn hoặc bằng `PHIEUDANGKY.TongTienPhaiDong` thì `DuDieuKienThi = TRUE`. Hệ thống cung cấp `fn_rbtv35_bat_dau_dong_hoc_phi(SoPhieu)`, `fn_rbtv35_du_dieu_kien_thi_hoc_phi(SoPhieu)` và view `V_DIEUKIENDUTHI_HOCPHI`; nếu bổ sung bảng dự thi thì dùng hàm này để chặn lập danh sách dự thi cuối kỳ. | `INSERT/UPDATE PHIEUTHUHOCPHI`; `UPDATE HOCKY.NgayBatDauDongHocPhi/HanDongHocPhi`; `UPDATE PHIEUDANGKY.MaHocKy` |
...
| Người 4 | `triggers_tuition_payment.sql` | `RBTV28` - `RBTV35` | Trigger kiểm tra phiếu thu, tổng đã thu, số tiền còn lại, chặn sửa phiếu thu thành công, cảnh báo/xử lý quá hạn, điều kiện dự thi theo hạn đóng học phí, trigger xóa mềm/trạng thái |
